...
Sent requests are added to the workspace History panel. History records the actual resolved method and URL together with status, duration, or network error, while retaining the raw editable request template and scripts for replay. Replaying a history entry restores the raw request rather than baking collection/environment substitutions or pre-request mutations back into the editor. History entries do not currently retain or reselect the originating collection, so collection-variable resolution on replay uses whichever collection is selected at replay time. The workspace keeps the latest 100 history entries; entries can be deleted individually or cleared together.
