...
Workspace authorization can be defined at the collection, folder, or request level. Requests may choose **Inherit from parent**; resolution walks the selected leaf folder, its ancestors, and then the collection. The nearest explicit auth setting wins. An explicit **No auth** setting stops inheritance. Collection and folder auth supports bearer tokens, OAuth 2.0 access tokens, Basic auth, and API keys in headers or query parameters. OAuth currently accepts an already-issued access token and sends it as Bearer authorization; interactive authorization-code/client-credentials grant acquisition is not implemented in the API Client yet. History retains collection and folder identity so inherited-auth requests replay in their original context when that context still exists.
