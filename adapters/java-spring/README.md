...
Current source version: `0.5.0`. It targets Java 17+, Spring Boot 3, renderer contract v1, and the FlexDoc renderer 2.x line.
...
The Java package namespace is `com.prauga.flexdoc.spring`. The starter depends on `com.prauga.flexdoc:flexdoc-jvm:0.5.0`, whose JAR owns the version-matched `flexdoc.standalone.js` and `flexdoc.standalone.css` assets.
...
```bash
jar tf adapters/java-jvm/target/flexdoc-jvm-0.5.0.jar | grep META-INF/flexdoc
```
